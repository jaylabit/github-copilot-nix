...
readonly PACKAGE_REPO="github/copilot-cli"
...
get_current_version() {
  sed -n 's/.*"version": "\([^"]*\)".*/\1/p' sources.json | head -1
}
...
fetch_latest_version() {
  ...
  curl "${curl_args[@]}" "$API_LATEST_URL" | sed -n 's/.*"tag_name": *"v\{0,1\}\([^"]*\)".*/\1/p' | head -1
}
...
download_checksums() {
  ...
  curl "${curl_args[@]}" "${RELEASE_BASE_URL}/v${version}/SHA256SUMS.txt" -o "$output"
}
...
hash_for_asset() {
  ...
  hex_hash=$(awk -v file="${asset_name}.tar.gz" '$2 == file { print $1 }' "$checksums_file")
  if [ -z "$hex_hash" ]; then
    log_error "No checksum found for ${asset_name}.tar.gz"
    return 1
  fi
...
write_sources_json() {
...

  {
    ...
    for index in "${!SYSTEMS[@]}"; do
      ...
      local hash
      hash=$(hash_for_asset "$checksums_file" "$asset_name")
...
update_to_version() {
  ...
  download_checksums "$new_version" "$checksums_file"
  write_sources_json "$new_version" "$checksums_file"
  update_flake_lock
  verify_build
}
...
main() {
  ...
  log_info "Successfully updated github-copilot-cli from $current_version to $latest_version"
